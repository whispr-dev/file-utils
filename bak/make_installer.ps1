...
try {
    ...
    $result = Start-Process -FilePath ".\7za.exe" -ArgumentList "a", "-t7z", "archive.7z", "file-utils-i.exe", "README", "LICENSE", "docs\", "install.bat", "UNWISE.bat" -Wait -NoNewWindow -PassThru -requireAdmin
...
$installerContent = @"
# file-utils-i Self-Extracting Installer v0.3.1
...
Write-Host "                    file-utils-i v0.3.1                         " -ForegroundColor Cyan
...
try {
    ...
    `$fileUtilsPaths = @(
        (Get-Command "file-utils-i.exe" -ErrorAction SilentlyContinue)?.Source,
        "`$env:ProgramFiles\file-utils-i\file-utils-i.exe",
        "`$env:ProgramFiles(x86)\file-utils-i\file-utils-i.exe",
        "`$env:LOCALAPPDATA\file-utils-i\file-utils-i.exe",
        "C:\tools\file-utils-i.exe"
    ) | Where-Object { `$_ -and (Test-Path `$_) }
    
    if (`$fileUtilsPaths) {
        Write-Host "file-utils-i.exe found at: `$(`$fileUtilsPaths[0])" -ForegroundColor Green
        
        # Try to get version info
        try {
            `$versionInfo = & `$fileUtilsPaths[0] --version 2>&1
            if (`$versionInfo) {
...
"@
    ...
    $installerContent | Out-File -FilePath "file-utils-i_installer.ps1" -Encoding UTF8
    
    if (Test-Path "file-utils-i_installer.ps1") {
        $ps1Size = [math]::Round((Get-Item "file-utils-i_installer.ps1").Length / 1024, 1)
...
try {
    ...
    # Check for icon
    $iconExists = Test-Path "file-utils-i.ico"
    Write-Host "Icon file (file-utils-i.ico) exists: $iconExists" -ForegroundColor Gray
    
    if ($iconExists) {
        $iconSize = [math]::Round((Get-Item "file-utils-i.ico").Length / 1024, 1)
        Write-Host "Icon size: $iconSize KB" -ForegroundColor Gray
    }
    
    # Clean up old EXE
    if (Test-Path "file-utils-i_installer.exe") {
        Write-Host "Removing existing EXE..." -ForegroundColor Gray
        Remove-Item "file-utils-i_installer.exe" -Force -ErrorAction SilentlyContinue
    ...
    if ($iconExists) {
        ...
        ps2exe `
            ...
            -outputFile "file-utils-i_installer.exe" `
            -iconFile "file-utils-i.ico" `
        ...
        ps2exe `
            -inputFile "file-utils-i_installer.ps1" `
            -outputFile "file-utils-i_installer.exe" `
            -title "file-utils-i Installer" `
            -description "Quantum-Enhanced File Security Tool Installer" `
            -company "whispr.dev" `
    ...
    if (Test-Path "file-utils-i_installer.exe") {
        $exeSize = [math]::Round((Get-Item "file-utils-i_installer.exe").Length / 1024 / 1024, 2)
...
"@
        ...
        $batchWrapper | Out-File -FilePath "file-utils-i_installer.bat" -Encoding ASCII
...
###################
 ...
 Generate professional checksums
Write-Host "Generating checksums..." -ForegroundColor Yellow
$hash = Get-FileHash "file-utils-i_installer.exe" -Algorithm SHA256
$size = [math]::Round((Get-Item "file-utils-i_installer.exe").Length / 1024 / 1024, 2)

@"
file-utils-i v0.3.0 - SHA256 Checksum
===================================
Generated: $(Get-Date -Format "yyyy-MM-dd HH:mm:ss UTC")
Publisher: whispr.dev

Verify with: Get-FileHash file-utils-i_installer.exe
...
$outputFiles = @("file-utils-i_installer.exe", "file-utils-i_installer.ps1", "file-utils-i_installer.bat", "archive.7z")
